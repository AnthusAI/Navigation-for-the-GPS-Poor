"""
Create the baseline trajectory visualization using the correct baseline flight data.
This ensures we're showing the actual baseline model performance.
"""
import sys
sys.path.append('../../..')
...
def create_baseline_trajectory_full():
    """Create full trajectory plot for baseline model using correct data."""
    print("--- Creating Baseline Model Full Trajectory ---")
    ...
    ax.text(0.5, 0.04, 'Baseline CNN Terrain Navigation: Individual Position Predictions',
            transform=ax.transAxes, fontsize=14, fontweight='bold',
            ha='center', va='bottom', color='white',
            bbox=dict(boxstyle='round,pad=0.4', facecolor='black', alpha=0.8, edgecolor='white'))

    # Save - overwrite the existing baseline trajectory
    output_path = '../images/predicted_vs_ground_truth_trajectory.png'
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plt.savefig(output_path, dpi=300, bbox_inches='tight', pad_inches=0)
    print(f"✅ Baseline full trajectory saved to {output_path}")
    plt.close()
    ...
    print(f"Predictions within 100px: {np.sum(errors < 100)}/150 ({(np.sum(errors < 100)/150)*100:.1f}%)")

if __name__ == "__main__":
    create_baseline_trajectory_full()